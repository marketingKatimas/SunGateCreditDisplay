...
//Payday Loan
const PaydayLoanDetail = document.getElementById('PaydayLoanDetail');
const PaydayLoanDetailDes = document.getElementById('PaydayLoanDetailDes');
...
function updateTitle(language) {
  ...
  function updatePaydayLoan(language) {
    if (language === 'bm') {
      PaydayLoanDetail.textContent = 'Pembiayaan Hari Gaji';
      PaydayLoanDetailDes.textContent = ' Bagi situasi tidak diduga, pembiayaan hari gaji diwujudkan untuk memberi anda bantuan kewangan segera. Tangani situasi mencabar dengan pembiayaan pantas dan selamat sehingga hari gaji seterusnya.';
      ApplyNowServices.textContent = 'Mohon Sekarang';
    } else {
      PaydayLoanDetail.textContent = 'Payday Loan';
      PaydayLoanDetailDes.textContent = 'For unexpected situations, our payday loans exist to provide instant financial relief that you need. Navigate through challenging situations with fast and accessible loans until your next paycheck.';
